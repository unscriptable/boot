...
function createNpmPackageDescriptor (options, url, name, meta) {
	var parts;
...
function processMetaFile (context, options, pkgName) {
	...
	return context.loader.import(url)
		.then(function (metadata) {
			...
			// TODO: deal with multiple versions via npm's versioning-by-nesting
...
function savePackageDescriptor (context, options, url, pkgName, metadata) {
	...
	context.packages[metadata.name] =
		createNpmPackageDescriptor(options, url, pkgName, metadata);
	return metadata;
}
